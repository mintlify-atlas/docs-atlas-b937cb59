...
description: Common issues and solutions when working with Vite
---
...
In Node.js versions 22 and below, ESM files cannot be loaded by [`require`](https://nodejs.org/docs/latest-v22.x/api/esm.html#require) by default.
...
<Accordion title="Solution: Increase file descriptor limit">
...
<Accordion title="Solution: Increase inotify limits">
...
<Accordion title="Solution: Update systemd config">
...
<Accordion title="macOS: Install trusted cert">
...
<AccordionGroup>
  <Accordion title="Version skew">
    ...
    - **Implement a graceful fallback**: Implement error handling for dynamic imports to reload the page when chunks are missing. See [Load Error Handling](/guides/build#load-error-handling) for more details.
  </Accordion>

  <Accordion title="Poor network conditions">
  ...
  <Accordion title="Browser extensions blocking requests">
