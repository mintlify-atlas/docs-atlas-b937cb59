...
description: Build server-side rendered applications with Vite's native SSR support
---
...
<Steps>
  <Step title="Create Vite server in middleware mode">
    ```js server.js
  ...
  <Step title="Implement server-rendered HTML handler">
...
<Note>
  The `--ssr` flag indicates this is an SSR build. It should also specify the SSR entry.
</Note>
