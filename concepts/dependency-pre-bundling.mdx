...
description: Understand how Vite pre-bundles dependencies to improve page loading speed and ensure CommonJS/UMD compatibility
---
...
During development, Vite serves all code as native ESM. Therefore, Vite must convert dependencies that are shipped as CommonJS or UMD into ESM first.
...
Both `include` and `exclude` can be used to deal with this:
...
<Tabs>
  <Tab title="Include">
    ...
    export default defineConfig({
      optimizeDeps: {
        include: ['large-esm-dep', 'commonjs-dep'],
      },
    })
    ```
  </Tab>
  <Tab title="Exclude">
    If the dependency is small and is already valid ESM, you can exclude it and let the browser load it directly.
...
### Dependency Updates Not Reflecting

If you've updated a dependency but changes aren't appearing:

1. Stop the dev server
2. Delete `node_modules/.vite`
3. Restart with `vite --force`

### Large Number of Modules

If a dependency has many internal modules causing slow page loads, add it to `optimizeDeps.include` to pre-bundle it into a single module.

### CommonJS Dependencies

If you encounter issues with CommonJS dependencies not being properly converted to ESM, ensure they're being pre-bundled by checking the `node_modules/.vite` directory or adding them to `optimizeDeps.include`.
