---
title: Why Vite
description: The problems Vite solves and why it exists
---
...
Vite [pre-bundles dependencies](/features#npm-dependency-resolving-and-pre-bundling) using [esbuild](https://esbuild.github.io/). **esbuild is written in Go and pre-bundles dependencies 10-100x faster than JavaScript-based bundlers.**
...
<Note>
In a bundler-based setup, the entire application must be crawled and built before serving. With Vite, the dev server starts instantly and modules are transformed on-demand.
</Note>
...
While Vite leverages esbuild to [pre-bundle dependencies](/features#npm-dependency-resolving-and-pre-bundling), Vite does not use esbuild as a bundler for production builds.
...
[WMR](https://github.com/preactjs/wmr) by the Preact team provided a similar feature set, and Vite's universal Rollup plugin API for dev and build was inspired by it. WMR is no longer maintained. **The Preact team now recommends Vite** with [@preactjs/preset-vite](https://github.com/preactjs/preset-vite).
...
Now that you understand why Vite exists and the problems it solves, you're ready to start building:
